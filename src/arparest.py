...
def arparest(x, p, method):
    ...
    if False: # switch 
        ...
        msgobj = 'signal:arparest:UnknMethod'
        msg = msgobj
        return []
    
    #% Do some data sanity testing
    if x==None or len(x)<minlength_x or max(mx, nx) > 1.:
        if method == 'modified':
            msgobj = 'signal:arparest:TooSmallForModel', 'X', '3/2'
            msg = msgobj
        else:
            msgobj = 'signal:arparest:TooSmallForModel', 'X', '2'
            msg = msgobj
    ...
    if issparse(x):
        msgobj = 'signal:arparest:InputSignalCannotBeSparse'
        msg = msgobj
        return []
    
    
    if p==None or p != np.round(p):
        msgobj = 'signal:arparest:ModelOrderMustBeInteger'
        msg = msgobj
        return []
